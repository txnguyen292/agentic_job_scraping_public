...
2. Count likely card units first using selectors such as `[data-search--pagination-target="jobCard"]`, `.job-card`, or a parent element containing `data-search--job-selection-*` markers. Record the selector, observed count, and expected emitted count in session context before writing helper scripts or protocol outputs.
...
- Do not finalize after a count mismatch. Repair the repeated-card evidence loading, the extraction output/run record, or the serialization helper so `output/candidates.json` and `output/final.json` represent one job per repeated card unit.
...
- If a card has `/sign_in?job=<job-slug>-NNNN...`, the output or serialization helper must convert it to `https://itviec.com/it-jobs/<job-slug>-NNNN`.
...
- If the page contains `.job-card` or `data-search--job-selection` markers but the output emits zero jobs, repair evidence loading, output/run record, or serialization instead of reporting "no stable job-card evidence".
...
- A finalize error shaped like "ITviec listing evidence expects N jobs but candidates.jobs has M" means the repeated-card evidence and emitted jobs disagree. Repair the card evidence loop, output/run record, or serialization helper; do not summarize a blocker until a distinct evidence/output repair has been attempted or a runtime guardrail stops execution.
- If page profiling records 20 repeated ITviec listing cards and the output emits 20 records, do not call 20 "too broad" by judgment alone. Validate URL shape, required fields, and expected fixture/reference content with `scripts/validate_outputs.py` or `scripts/sandbox_finalize.py`; repair only from a concrete validator/finalizer error or directly inspected malformed records.
