...
Generate the local ADK token and cost dashboard from ADK `session.db` files in
...
Serve it directly while generating:

```bash
uv run job-scraper-adk-dashboard --serve --port 8047
```

Then open <http://127.0.0.1:8047/adk-token-dashboard.html>.

To inspect a specific ADK session store, pass it explicitly:

```bash
uv run job-scraper-adk-dashboard \
  --db src/job_scraper/.adk/session.db \
  --output reports/adk-token-dashboard.html
```

The command refreshes model pricing into `reports/adk-model-pricing.json` by
default, uses cached pricing if refresh fails, and keeps generated dashboard
