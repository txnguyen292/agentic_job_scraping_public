...
root_agent = Agent(
    ...
    instruction=(
        ...
        "`initial_plan` is the bootstrap workflow guide, not the extraction method. Once `extraction_plan` exists, "
        "do not treat `initial_plan` as live guidance. `extraction_plan` is the adaptive plan. After inspecting "
        "the first representative repeated job unit or evidence chunk, update_extraction_context with `extraction_plan` and "
        "`extraction_strategy` as the detailed method derived from `extraction_plan`: target_units, unit_boundary, "
        "count_method, field_patterns, known_exclusions, coverage_plan, and why this strategy follows from the "
        "observed evidence. Follow that strategy by default, enhance it when new evidence adds useful field/pattern "
        "detail, and revise it when new evidence or validation/finalization contradicts it. Before writing or "
        "running producer scripts such as output/extractor.py, `immediate_goal` must name the current step inside "
        "`extraction_strategy` with evidence, strategy, validation, and next script/probe objective. For the first "
        "fixture-backed ITviec goal, establish the repeated job-card unit boundary and validate that boundary before "
        "extracting fields or claiming persistence. Good immediate_goal example: "
        "Establish repeated job-card unit boundary for the fixed ITviec fixture. "
        "Evidence: fixed page artifact, representative repeated card markup/text, and bounded selector/count evidence. "
        "Strategy: target one repeated job-card unit per in-scope listing using "
        "[data-search--pagination-target='jobCard'] and exclude navigation/company preview links. "
        "Validation: run a bounded count probe and pass only when the count is 20 for the fixture. "
        "Next script objective: write the smallest probe that counts repeated job units and records the unit boundary. "
        ...
        "`extraction_plan`, `extraction_strategy`, `immediate_goal`, and `expected_output`. The immediate_goal must "
        "define the current bounded step with evidence, strategy, validation, and next script/probe objective. "
        "The expected_output must declare the job count implied by repeated-pattern "
